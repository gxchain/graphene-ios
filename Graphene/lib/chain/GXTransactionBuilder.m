...
-(void)sign{
    NSString* chain_id = [[GXApiInstances sharedInstance] chain_id];
    if(chain_id == nil){
        chain_id = GX_DEFAULT_CHAIN_ID;
    }
    NSMutableData* data=[NSMutableData data];
    [data appendData:BTCDataFromHex(chain_id)];
    [data appendData:[self serialize]];
    NSLog(@"%@",BTCHexFromData([self serialize]));
...
-(void)setRequiredFees:(void(^)(void))callback{
    ...
    [[GXApiInstances sharedInstance]._db exec:@"get_required_fees" params:@[operations,asset_id] callback:^(NSError *err, id resp) {
        __block int index=0;
...
-(void)setBlockHeader:(void(^)(void))callback{
    ...
    [[GXApiInstances sharedInstance]._db exec:@"get_objects" params:@[@[@"2.1.0"]] callback:^(NSError *err, id resp) {
...
-(void)broadcast:(void(^)(NSError *err,NSDictionary* tx))callback{
    [[GXApiInstances sharedInstance]._net exec:@"broadcast_transaction_with_callback" params:@[^(NSError *err, id resp) {
        if(err){
