...
@interface GXPublicKey()

@end

@implementation GXPublicKey
+(instancetype)fromString:(NSString*)pubKeyString{
    NSString* prefix = [pubKeyString substringToIndex:[GX_ADDRESS_PREFIX length]];
    NSAssert([prefix isEqualToString:GX_ADDRESS_PREFIX], @"Expecting key to begin with $%@, instead got %@",GX_ADDRESS_PREFIX,prefix);
    NSMutableData* pubKeyData = BTCDataFromBase58CheckRIPEMD160([pubKeyString substringFromIndex:[prefix length]]);
    return [GXPublicKey fromData:pubKeyData];
}

+(instancetype)fromData:(NSData*)pubKeyData{
    GXPublicKey* instance = [[GXPublicKey alloc]init];
...
-(NSString*)toString{
    return [NSString stringWithFormat:@"%@%@",GX_ADDRESS_PREFIX,BTCBase58CheckStringWithDataRIPEMD160([self publicKeyWithCompression:YES])];
}
