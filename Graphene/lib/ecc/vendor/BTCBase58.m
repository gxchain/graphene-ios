...
NSMutableData* BTCDataFromBase58CheckRIPEMD160(NSString* string) {
    return BTCDataFromBase58CheckCStringRIPEMD160([string cStringUsingEncoding:NSASCIIStringEncoding]);
}
...
NSMutableData* BTCDataFromBase58CheckCStringRIPEMD160(const char* cstring) {
    if (cstring == NULL) return nil;
    ...
    // GRAPHENE uses RIPEMD160 instead of SHA256 in public key
    NSData* hash = BTCRIPEMD160([result subdataWithRange:NSMakeRange(0, length - 4)]);
