...
@implementation GXPrivateKey
+(instancetype)fromWif:(NSString*)wifKey{
    GXPrivateKey* instance = [[GXPrivateKey alloc]init];
...
-(NSData*)sharedSecret:(GXPublicKey*)publicKey{
    ...
    publicKey= [GXPublicKey fromData:[publicKey publicKeyWithCompression:NO]];
        ...
        if (nBitsR <= 256 && nBitsS <= 256)
        {
            ...
            for (int i=0; i<4; i++)
            {
                if (ECDSA_SIG_recover_key_GFp(key, sig, hashbytes, hashlength, i, 1) == 1) {
                    GXPublicKey* _pubkey=[GXPublicKey new];
                    _pubkey._key=key;
