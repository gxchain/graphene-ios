...
@interface GXConnectionManager()
@property (nonatomic,strong) GXApiInstances* Api;
@end

@implementation GXConnectionManager

+(instancetype)sharedInstance{
    static dispatch_once_t onceToken;
    static GXConnectionManager* instance;
    dispatch_once(&onceToken, ^{
        instance=[[GXConnectionManager alloc] init];
        instance.Api=[GXApiInstances sharedInstance];
    });
    return instance;
}
...
-(void)checkConnect:(NSString*)url callback:(void(^)(NSArray* info))callback{
    GXChainWebSocket* conn = [[GXChainWebSocket alloc]initWithAddress:url];
